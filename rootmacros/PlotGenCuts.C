...
void PlotGenCuts(const char* file1 = "dvcs_cuts_gen_bin.csv",
                 const char* file2 = "dvcs_cuts_gen.csv") {
  gStyle->SetOptStat(0);
  ...
  for (int cfg = 0; cfg <= 2; cfg++) {
    ...
    TString outPdf = Form("cuts_Gencfg%d.pdf", cfg);
    TString outPng = Form("cuts_Gencfg%d.png", cfg);
    c->SaveAs(outPdf);
    c->SaveAs(outPng);
  }
