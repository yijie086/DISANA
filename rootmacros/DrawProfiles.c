...
#include <TCanvas.h>
#include <TLegend.h>
#include <TLatex.h> // 添加头文件以使用 TLatex
#include <iostream>
#include <string>
#include <vector> // 添加头文件以使用 std::vector
...
void DrawProfiles() {
    ...
    for (int sector = 1; sector <= 6; ++sector) {
        ...
        std::vector<std::string> profile_names = {
            "theta5to10_sector" + std::to_string(sector) + "_chi2perndf_vs_edge_profile",
            "theta10to15_sector" + std::to_string(sector) + "_chi2perndf_vs_edge_profile",
            "theta15to20_sector" + std::to_string(sector) + "_chi2perndf_vs_edge_profile",
            "theta20to25_sector" + std::to_string(sector) + "_chi2perndf_vs_edge_profile"
        };
        ...
        // 创建画布
        TCanvas* canvas = new TCanvas(("canvas_sector" + std::to_string(sector)).c_str(),
                                      ("Chi2/NDF vs Edge Sector " + std::to_string(sector)).c_str(),
                                      1920, 1080);
        canvas->SetGrid();
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            profile->GetYaxis()->SetRangeUser(0, 150);
            profile->SetStats(0);
        ...
        latex.DrawLatexNDC(0.5, 0.95, ("electron Chi2/NDF vs Edge sector " + std::to_string(sector)).c_str());

        // 保存画布为图片
        canvas->SaveAs(("chi2perndf_vs_edge_profiles_sector" + std::to_string(sector) + ".png").c_str());
