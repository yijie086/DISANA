...
void DISANA_Xplotter2() {
  ...
  std::string input_path_from_analysisRun_data = "./../build/data7546";
  std::string input_path_from_analysisRun_MC = "./../build/dvcsmc7546";

  std::string filename_afterFid_data = Form("%s/dfSelected_afterFid.root", input_path_from_analysisRun_data.c_str());
  std::string filename_afterFid_MC = Form("%s/dfSelected_afterFid.root", input_path_from_analysisRun_MC.c_str());
  float beam_energy = 7.546;

  ROOT::RDF::RNode df_afterFid_data = InitKinematics(filename_afterFid_data, "dfSelected_afterFid", beam_energy);
  ROOT::RDF::RNode df_afterFid_MC = InitKinematics(filename_afterFid_MC, "dfSelected_afterFid", beam_energy);
  ...
  auto df_final_dvcs_data = ApplyFinalDVCSSelections(df_afterFid_data, true);
  auto df_final_dvcsPi_rejected_data = RejectPi0TwoPhoton(df_final_dvcs_data);
  auto df_final_dvcs_MC = ApplyFinalDVCSSelections(df_afterFid_MC, true);
  auto df_final_dvcsPi_rejected_MC = RejectPi0TwoPhoton(df_final_dvcs_MC);

  // pi0 event selection cuts
  auto df_final_OnlPi0_data = SelectPi0Event(df_final_dvcs_data);
  auto df_final_OnlPi0_MC = SelectPi0Event(df_final_dvcs_MC);
  ...
  //xBins.SetQ2Bins({1.0, 1.5, 2.0, 3.0, 5.0});
  //xBins.SetTBins({0.25, 0.4});
  //xBins.SetXBBins({0.07, 0.10, 0.15, 0.20, 0.28, 0.40});
  ...
  comparer.AddModel(df_final_dvcsPi_rejected_data, "Fall2018 7.546GeV", beam_energy);
  //comparer.AddModel(df_final_dvcsPi_rejected_MC, "Fall2018 7.546GeV MC", beam_energy);
...
ROOT::RDF::RNode ApplyFinalDVCSSelections(ROOT::RDF::RNode df, bool inbending) {
  return df
      ...
      .Filter("Theta_e_gamma > 5.0", "Cut: Theta_e_gamma > 5.0")
