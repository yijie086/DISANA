...
void DISANA_Xplotter2() {
  ...
  bool DoBkgCorr = false;       // Set to true if you want to apply background correction

  std::string input_path_from_analysisRun_out_data = "/work/clas12/yijie/clas12ana/analysis308/DISANA/build/full";
  ...
  //comparer.AddModel(df_final_dvcsPi_rejected_outb_data, "Fall18 7.546GeV C", beam_energy, DoBkgCorr, "correction_factorsOutb.root");
  comparer.AddModel(df_final_dvcsPi_rejected_outb_data, "Fall18 7.546GeV", beam_energy);
  //comparer.PlotKinematicComparison();
  //comparer.PlotDVCSKinematicsComparison();
  double luminosity = 2.4*10000000*M_PI/180;  // Set your desired luminosity here
...
void CreateCorrectionHistogram4D(ROOT::RDF::RNode df_dvcs_mc, ROOT::RDF::RNode df_pi0_mc, ROOT::RDF::RNode df_dvcs_data, ROOT::RDF::RNode df_pi0_data,
  ...
  // Define binning
  const int nQ2Bins = 1, nTBins = 1, nXBBins = 1, nPhiBins = 18;
