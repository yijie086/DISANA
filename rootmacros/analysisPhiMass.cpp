...
static inline void MakeDir(const std::string& dir) { gSystem->Exec(std::string("mkdir -p \"" + dir + "\"").c_str()); }
...
// ------------------------
inline void FitAndDrawHistogram(TH1D* hist, const std::string& title, const std::string& outname, TString xtitle = "M(K^{+}K^{-}) [GeV]", TString ytitle = "Counts",
  ...
  if (fit) {
    // Replace fitTotal definition with a normalized signal term:
    /*TF1* fitTotal = new TF1(("fitTotal_" + safe).c_str(),
      // background density + signal density
      "[0]*TMath::Power(x-0.9874,[1])*TMath::Exp(-[2]*(x-0.9874))" // bkg density term
      " + [3]*TMath::Gaus(x,[4],[5],true)",                        // signal: Nsig * normalized Gauss
    ...
    // bin width is the width of the bin [0] is amplitude (# of phi), [1] is guassian mean, [2] is gaussian width 0.398942 = 1/sqtr(2*pi)
    ...
    TF1* fitSignal =
        new TF1(("fitSignal_" + safe).c_str(), Form("[0]*0.398942*%g*TMath::Exp(-0.5*TMath::Power((x-[1])/([2]),2))/TMath::Abs([2])", bw), fit_range_min, fit_range_max);
    ...
    if (annotate_sigma_for_count > 0 && std::isfinite(mu) && std::isfinite(sigma) && sigma > 0) {
      const double k = annotate_sigma_for_count;
      const double frac = TMath::Erf(k / TMath::Sqrt2());  // two-sided fraction of a Gaussian
      const double Nsig_in_k = Nsig * frac;
      latex.DrawLatex(0.15, 0.67, Form("N_{#phi} (total) = %.1f", Nsig));
    }

    /*if (annotate_sigma_for_count > 0 && std::isfinite(mu) && std::isfinite(sigma) && sigma > 0) {
      const double lo = mu - annotate_sigma_for_count * sigma;
      const double hi = mu + annotate_sigma_for_count * sigma;
      const int binLo = hist->GetXaxis()->FindBin(lo);
      const int binHi = hist->GetXaxis()->FindBin(hi);

      // Raw counts in window (integer sum of bin contents)
      long long nRaw = 0;
      for (int b = binLo; b <= binHi; ++b) nRaw += (long long)std::llround(hist->GetBinContent(b));

      // Background estimate in the same window from the background TF1, bin-by-bin
      // (eval at bin center * bin width -> estimated counts in that bin)
      double nBkgEst = 0.0;
      double nSigEstFromFit = 0.0;  // signal-from-fit (bin-by-bin) if you want to show it
      for (int b = binLo; b <= binHi; ++b) {
        const double bw = hist->GetXaxis()->GetBinWidth(b);
        const double xc = hist->GetXaxis()->GetBinCenter(b);
        nBkgEst        += fitBkg->Eval(xc)    * bw;
        nSigEstFromFit += fitSignal->Eval(xc) * bw;  // optional: signal from fit in the window
      }

      // Background-subtracted count
      const double nSub = (double)nRaw - nBkgEst;
      const double eRaw = std::sqrt(std::max(0.0, (double)nRaw));
      const double eSub = std::sqrt(std::max(0.0, (double)nRaw + nBkgEst));

      latex.DrawLatex(0.15, 0.67, Form("N_{|M-#mu|<%.0f#sigma} (raw) = %lld #pm %.0f",
                                       annotate_sigma_for_count, nRaw, eRaw));
      latex.DrawLatex(0.15, 0.62, Form("N_{|M-#mu|<%.0f#sigma} (bkg-sub) = %.1f #pm %.1f",
                                       annotate_sigma_for_count, nSub, eSub));

      // Optional: show the signal-from-fit inside the same window (bin-by-bin)
      latex.DrawLatex(0.15, 0.57, Form("N_{sig}^{fit}(|M-#mu|<%.0f#sigma) #approx %.1f",
                                       annotate_sigma_for_count, nSigEstFromFit));
    }
    */
...
// ------------------------
void DrawPhiMass(const std::string& filename, const std::string& treename, const std::string& outputDir = "PhiMassPlots", const std::string& tag = "") {
  const std::string safe = MakeSafeName(tag);
  TStopwatch timer;
  timer.Start();
  ...
  auto df_withPK_only = df_withPKmPKp
                            .Define("PKm_mass",
                                    [](const RVec<std::pair<float, float>>& pairs) {
                                      RVec<float> r;
                                      for (auto& p : pairs) r.push_back(p.first);
                                      return r;
                                    },
                                    {"PKm_mass_vs_KK"})
                            .Define("PKp_mass",
  ...
  auto df_split2D = df_withPK_only
                        .Define("PKm_mass_X",
                                [](const RVec<std::pair<float, float>>& v) {
                                  RVec<float> x;
                                  for (auto& p : v) x.push_back(p.first);
                                  return x;
                                },
                                {"PKm_mass_vs_KK"})
                        .Define("PKm_mass_Y",
                                [](const RVec<std::pair<float, float>>& v) {
                                  RVec<float> y;
                                  for (auto& p : v) y.push_back(p.second);
                                  return y;
                                },
                                {"PKm_mass_vs_KK"})
                        .Define("PKp_mass_X",
  ...
  auto h_KKmass = df_selectedKaons.Histo1D({Form("hKKMass_%s", safe.c_str()), "After Mass Cut;M(K^{+}K^{-}) [GeV];Counts", 200, 0.80, 1.6}, "KK_mass");

  auto h2_PKm_vs_KK = df_split2D.Histo2D({Form("h2PKmKK_%s", safe.c_str()), "M(pK^{-}) vs M(K^{+}K^{-});M(K^{+}K^{-}) [GeV];M(pK^{-}) [GeV]", 200, .980, 2.5, 100, 1.4, 4.5},
                                         "PKm_mass_Y", "PKm_mass_X");

  auto h2_PKp_vs_KK = df_split2D.Histo2D({Form("h2PKpKK_%s", safe.c_str()), "M(pK^{+}) vs M(K^{+}K^{-});M(K^{+}K^{-}) [GeV];M(pK^{+}) [GeV]", 200, 0.980, 2.5, 100, 1.4, 4.5},
                                         "PKp_mass_Y", "PKp_mass_X");

  auto h_PKm = df_withPK_only.Histo1D({Form("hPKm_%s", safe.c_str()), "Invariant Mass of pK^{-};M(pK^{-}) [GeV];Counts", 200, 1.2, 2.6}, "PKm_mass");

  auto h_PKp = df_withPK_only.Histo1D({Form("hPKp_%s", safe.c_str()), "Invariant Mass of pK^{+};M(pK^{+}) [GeV];Counts", 200, 1.2, 2.6}, "PKp_mass");

  MakeDir(outputDir);

  FitAndDrawHistogram(h_KKmass.GetPtr(), "After InvMass Cut", outputDir + "/KpKm_mass_after", "M(K^{+}K^{-}) [GeV]", "Counts", fit_range_min, fit_range_max, true, tag,
                      /*annotate_sigma_for_count=*/3.0);

  FitAndDrawHistogram(h_PKm.GetPtr(), "After InvMass Cut", outputDir + "/pKm_mass_after", "M(pK^{-}) [GeV]", "Counts", 1.0, 4.0, false, tag, /*annotate_sigma_for_count=*/3.0);

  FitAndDrawHistogram(h_PKp.GetPtr(), "After InvMass Cut", outputDir + "/pKp_mass_after", "M(pK^{+}) [GeV]", "Counts", 1.0, 4.0, false, tag, /*annotate_sigma_for_count=*/3.0);

  Draw2DHistogram(h2_PKm_vs_KK.GetPtr(), "M(K^{+}K^{-}) vs M(pK^{-})", outputDir + "/pKm_vs_KK", "M(K^{+}K^{-}) [GeV]", "M(pK^{-}) [GeV]", 0.98, 2.2, 1.3, 2.8);

  Draw2DHistogram(h2_PKp_vs_KK.GetPtr(), "M(K^{+}K^{-}) vs M(pK^{+})", outputDir + "/pKp_vs_KK", "M(K^{+}K^{-}) [GeV]", "M(pK^{+}) [GeV]", 0.98, 2.2, 1.3, 2.8);
...
void analysisPhiMass() {
  ...
  std::string filename_afterFid_SP19inb_data = Form("%s/dfSelected.root", input_path_from_analysisRun_SP19inb_data.c_str());
  ...
  std::vector<Job> jobs = {/*{ filename_afterFid_SP18inb_data,  "dfSelected_afterFid", baseOut + "/spring2018/inb",  "Spring 2018 INB"  },
                           ...
                           {filename_afterFid_SP19inb_data, "dfSelected", baseOut + "/spring2019/inb", "Spring 2019 INB"}};
