...
get_RECParticle_float_var(int target_pid, int target_charge) {
    return [target_pid, target_charge](const std::vector<float>& var, const std::vector<int>& pid, const std::vector<int>& charge, const std::vector<float>& p) {
        std::vector<float> out;
        for (size_t i = 0; i < pid.size(); ++i) {
            if (pid[i] == target_pid && static_cast<int8_t>(charge[i])==target_charge && p[i] > 0.02) {
                out.push_back(var[i]);
            }
        }
        return out;
...
get_RECParticle_int_var(int target_pid, int target_charge) {
    return [target_pid, target_charge](const std::vector<int>& var, const std::vector<int>& pid, const std::vector<int>& charge, const std::vector<float>& p) {
        std::vector<int> out;
        for (size_t i = 0; i < pid.size(); ++i) {
            if (pid[i] == target_pid && static_cast<int8_t>(charge[i])==target_charge && p[i] > 0.02) {
                out.push_back(var[i]);
