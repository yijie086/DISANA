...
static ROOT::RDataFrame makeRDFFromSlice(std::vector<std::string> slice) {
  auto ds = std::make_unique<RHipoDS>(std::move(slice));
...
inspectSliceSchema(const std::vector<std::string>& slice) {
  ...
  if (n == 0) {
    std::cerr
        << "[Converter] Schema discovery: slice has 0 rows, ignoring in intersection\n";
    return {};
  }

  std::unordered_map<std::string, std::string> types;
  for (const auto& c : rdf.GetColumnNames()) {
    types.emplace(c, rdf.GetColumnType(c));
  }
  return types;
}
...
computeStableColumns(const std::vector<std::vector<std::string>>& slices) {
  ...
  if (!haveSeed) {
    throw std::runtime_error(
        "[Converter] All slices appear empty; cannot determine stable schema.");
  }
...
HipoToRootConverter::getHipoFilesInPath(const std::string& pathOrPattern,
                                        int nfiles) const {
...
std::vector<std::string> HipoToRootConverter::convert(
  ...
  if (hipoFiles.empty()) {
    std::cerr << "[Converter] No .hipo files found in: " << fInputDir_
              << std::endl;
    return {};
  }
  ...
  const std::size_t nWorkers = std::max<std::size_t>(
      1, std::min<std::size_t>(
             fnThreads_ > 0 ? static_cast<std::size_t>(fnThreads_) : 1,
             hipoFiles.size()));

  const std::size_t filesPerThread =
      (hipoFiles.size() + nWorkers - 1) / nWorkers;
  ...
  if (stableCols.empty()) {
    throw std::runtime_error(
        "[Converter] No common, type-consistent columns across NON-empty slices.");
  }
  ...
  for (std::size_t i = 0; i < slices.size(); ++i) {
    std::string tmp =
        (fs::path(fOutputDir_) /
         (tempFilePrefix + std::to_string(i) + ".root"))
            .string();
  ...
  for (auto& f : tempRoots) {
    std::error_code ec2;
    if (fs::exists(f, ec2) && !ec2 && fs::file_size(f, ec2) > 0 && !ec2) {
      pruned.push_back(f);
    } else {
